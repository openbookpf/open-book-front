...
function Cart() {
    ...
    // bg-[#fef3ed]
    return (
        <div className="flex mv:flex-col sm:justify-start md:space-x-10 md:px-10 lg:justify-between md:items-start sm:items-center mv:items-center md:flex-row font-poppins pb-5 my-10 md:mx-auto min-w-screen">
            <div className="flex-col py-10 grow">
                {cartProducts && cartProducts.length > 0 ? (
                    <div className="flex flex-col space-y-4 ">
                        {cartProducts.map((p) => (
                            <div key={p.ISBN} className=" bg-[#fef3ed] flex items-center p-4 shadow-lg rounded-lg">
                                <img
                                ...
                                <div className="ml-4 w-40">
                                    ...
                                    <Link to="">
                                        <p className="text-gray-800 truncate font-bold ">{p.book_title}</p>
                                    </Link>
                                </div>
            ...
            <div className="flex flex-col jus mt-10 h-64 space-y-4 bg-[#fef3ed] py-14 px-15 mv:w-[300px] mv:min-w-[200px] lg:w-[400px] items-center shadow-lg rounded-lg">
                <div className="font-bold text-lg">Total books: {Math.max(0, cartCounter)}</div>
                <div className="font-bold text-lg">Total price: ${Math.max(0, cartTotalPrice).toFixed(2)}</div>
                {cartCounter > 0 ? (
                    <Link
                        to={isAuthenticated ? "/checkout" : "#"}
                        title={`${isAuthenticated ? "" : "log in to buy books"}`}
                        className={`${
                            isAuthenticated ? "bg-cyan-0 hover:bg-green-700" : "bg-blue-0 bg-opacity-25"
